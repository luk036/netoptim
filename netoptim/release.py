"""Release data for NetOptim.

When NetOptim is imported a number of steps are followed to determine
the version information.
   ...
   1) If the release is not a development release (dev=False), then version
      ...
      downloadable release of NetOptim since setup.py creates it during
      packaging/installation.  However, version.py might not exist if one
      is running NetOptim from the mercurial repository.  In the event that
...
NetOptim, we must make sure write_versionfile() does not overwrite the
...
def write_versionfile():
    ...
    text = '''"""
Version information for NetOptim, created during installation.
...
# Was NetOptim built from a development version? If so, remember that the major
...
def get_info(dynamic=True):
    ...
    if dynamic_failed or not dynamic:
        # This is where most final releases of NetOptim will be.
...
long_description = \
    """
NetOptim is a Python package for the creation, manipulation, and
study of the structure, dynamics, and functions of projective geometry.

"""
license = 'BSD'
authors = {'luk036': ('Wai-Shing Luk', 'luk036@gmail.com')}
maintainer = "NetOptim Developers"
maintainer_email = "netoptim-discuss@googlegroups.com"
url = 'http://netoptim.github.io/'
download_url = 'https://pypi.python.org/pypi/netoptim/'
